...
/* ── POST /api/smp/issued ────────────────────────────────────────────── */
router.post('/', requireAuth, requireRole('admin','storekeeper'), async (req, res) => {
  ...
  const record = await db.insert('issued', {
...
router.get('/', requireAuth, async (req, res) => {
  let records = await db.findAll('issued')
...
router.get('/:id', requireAuth, async (req, res) => {
  const r = await db.findById('issued', req.params.id)
