  ...
  return (
    <>
      ...
      <motion.div
        ...
        style={{ background: PAGE_BG, height: '100dvh' }}
        variants={pageIn}
        initial="hidden"
        animate="visible"
      >
        ...
        >
          ...
          >
            ...
            <p style={{ ...SUBTITLE_STYLE, fontSize: 'clamp(0.58rem, 1.5vw, 0.78rem)' }} className="mb-0.5">
          ...
          {/* ── LANGUAGE CARDS ─────────────────────────────────────────────── */}
          {/* 3 col on all screens — clamp handles size; px-2 adds breathing room */}
          <div className="w-full max-w-2xl grid grid-cols-3 gap-2 sm:gap-4 md:gap-5 px-2 sm:px-2">
        ...
        >
          ...
          <div
            className="px-4 sm:px-6 pt-2.5 sm:pt-3"
            style={{
              background: '#4A0918',
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-y-1 sm:gap-y-0 sm:gap-x-8">
              {FOOTER_ITEMS.map(({ Icon, text }, i) => (
                <div key={i} className="flex items-center gap-2">
                  <Icon size={14} style={{ color: '#C79A2B', flexShrink: 0 }} />
                  <span style={FOOTER_TEXT}>{text}</span>
                </div>
              ))}
            </div>
            <p className="text-center mt-1.5" style={FOOTER_COPYRIGHT}>
  ...
  if (!imageError) {
    return (
      <img
        ...
        style={{
          width: 'clamp(180px, 55vw, 780px)',
          maxWidth: '100%',
          maxHeight: 'clamp(80px, 22vh, 200px)',
          height: 'auto',
          objectFit: 'contain',
          display: 'block',
        }}
...
const CENTER_GLOW = {
  background: 'radial-gradient(ellipse 60% 50% at 50% 45%, rgba(201,166,70,0.08) 0%, transparent 70%)',
}
